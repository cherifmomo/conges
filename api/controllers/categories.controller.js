...
exports.findAll = (req, res) => {
  ...
  Categories.find(condition).sort({'createdAt':-1})
    .then(data => {
      res.send(data);
    })
    .catch(err => {
...
exports.update = (req, res) => {
  ...
  Categories.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
    .then(data => {
      if (!data) {
        res.status(404).send({
          message: `Cannot update Categories with id=${id}. Maybe Employee was not found!`
        });
...
exports.delete = (req, res) => {
  ...
  Categories.findByIdAndRemove(id, { useFindAndModify: false })
    .then(data => {
      if (!data) {
        res.status(404).send({
          message: `Cannot delete Categories with id=${id}. Maybe Categories was not found!`
        });
      } else {
        res.send({
